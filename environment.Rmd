...
```{r echo=FALSE, purl=F}
xfun::embed_file('D:/R/Data/reef_fish.xlsx')
```
...
My file `` is an .xlsx file located in the file 'Data' of my working directory. 

```{r,  eval=T}
# 1. using the package `readxl`
# 2. reading my `.xlsx` in my working directory
# 3. importing `.xlsx` in a `fish` object
library(readxl) # load the package `readxl'
read_excel('D:/R/Data/reef_fish.xlsx') # automatically print on my screen
fish<-read_excel('D:/R/Data/reef_fish.xlsx') # store my table in an object called `fish`
...
fish<-read.table('D:/R/Data/reef_fish.txt', header=T, sep='\t', dec='.') 
```
...
fish<-read.table ("D:/R/Data/Data/.txt",header = TRUE,sep="\t", dec=".")# long version
fish<-read.table("D:/R/Data/Data/.txt", TRUE, "\t",".")# short version
```
...
<p class="comment">
**Practice 1.2** Download `.xlsx`, save it in a working directory you dedicated for this course, and import it in R. Do the same after converting this `` file into a `.txt` format
</p>
